...
isBloodOrange(Orange(name, _)) = name `elem` ["Tarocco", "Moro", "Sanguinello"]
isBloodOrange _ = False


-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments fruits = sum [seg fruit | fruit <- fruits, isBloodOrange fruit]
  where seg(Orange(_, n)) = n
        seg _ = 0

-- 3.
worms :: [Fruit] -> Int
worms fruits = length [fruit | fruit <- fruits, hasWorm fruit]
  where hasWorm(Apple(_, worm)) = worm
        hasWorm _ = False
...
-- 5.
tautology :: Prop -> Bool
tautology prop = and [ eval env prop | env <- envs (names prop)]
...
equivalent p1 p2 = and [ eval env p1 == eval env p2 | env <- envs names']
  where names' = nub (names p1 ++ names p2)
